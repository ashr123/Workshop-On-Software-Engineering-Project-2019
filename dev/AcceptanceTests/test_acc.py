...
class TestClass(object):
    ...
    def set_up1(self):
        ...
        self._serviceFacade.login(self._id1, "noa", "098765")
        self._serviceFacade.addStore(self._id1, self._store_1['name'], "Everything you need for your dog", [])
        self._item_1['id_DEPRECATED'] = self._serviceFacade.addItemToStore(self._id1, self._store_1['name'],
                                                                           self._item_1['name'],
                                                                           "Pets",
                                                                           "makes dogs fur shiny and soft", 13.5, 120)[
            1]
        self._item_2['id_DEPRECATED'] = self._serviceFacade.addItemToStore(self._id1, self._store_1['name'],
                                                                           self._item_2['name'], "Pets",
                                                                           "makes dogs fur shiny and soft", 12, 40)[1]
        self._item_3['id_DEPRECATED'] = self._serviceFacade.addItemToStore(self._id1, self._store_1['name'],
    ...
    def set_up21(self):
        ...
        self._store_2 = {'name': 'ZARA'}
        self._item_4 = {'name': "skirt", 'price': 85, 'store_name': self._store_2['name']}
        self._serviceFacade.addStore(self._id1, self._store_2['name'], "clothes shoes acc best price", [])
        self._item_4['id_DEPRECATED'] = self._serviceFacade.addItemToStore(self._id1, self._store_2['name'],
                                                                           self._item_4['name'], "Fashion",
                                                                           "great skirt", 85, 7)[1]
        self._serviceFacade.saveItemInCart(sessionId, self._item_4["name"], self._item_4["store_name"])
        return sessionId
    ...
    def test_buyManyItems2_only_buy_test(self):
        sessionId = self.set_up21()
        tosend = [
            {"store": self._item_1['store_name'], "list_of_items": [self._item_1['name'], self._item_2['name']]},
            {"store": self._item_4['store_name'], "list_of_items": [self._item_4['name']]}]
        trans_id = self._serviceFacade.buyManyItems(sessionId, tosend)
        assert self._serviceFacade.watch_trans(trans_id) == "price: {}".format(
            self._item_1['price'] + self._item_2['price'] + self._item_4['price'])
        assert self._serviceFacade.get_item_amount(self._item_1['store_name'], self._item_1['name']) == 119
        assert self._serviceFacade.get_item_amount(self._item_2['store_name'], self._item_2['name']) == 39
    ...
    def test_buyManyItems3_only_buy_test(self):
        sessionId = self.set_up2()
        tosend = [
    ...
    def test_buyManyItems4_only_buy_test(self):
        sessionId = self.set_up2()
    ...
    # 2.8.1 buy single item 7
    @pytest.mark.skip(reason="no way of currently testing this, no rules yet")
    def test_buyManyItems7(self):
        pass

    # 2.8.2 buy many items 8
    def test_buyManyItems8(self):
        sessionId, trans_id = self.set_up6()
        self._serviceFacade.make_collection_fail()
    ...
    def test_buyManyItems9(self):
        sessionId, trans_id = self.set_up5()
        self._serviceFacade.make_supply_fail()
        assert "supply system is down" == self._serviceFacade.supply(trans_id, 'Hakishon 12, Tel Aviv, 7514050, Israel')
        self._serviceFacade.make_supply_pass()
        assert self._serviceFacade.get_item_amount(self._item_1['store_name'], self._item_1['name']) == 120
        assert self._serviceFacade.get_item_amount(self._item_2['store_name'], self._item_2['name']) == 40
        assert self._item_1['name'] in self._serviceFacade.watchCart(sessionId) and self._item_2[
