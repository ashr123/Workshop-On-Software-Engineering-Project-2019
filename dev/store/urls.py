...
urlpatterns = [
	...
	path('add_item/<int:pk>', views.add_item),  # Not tested, is needed???
	path('add_item_2/', views.AddItemToStore.as_view()),  # Not tested?
	...
	path('update/<int:pk>/', views.StoreUpdate.as_view()),  # not ready yes
	path('add_store/submit/', views.submit_open_store),  # Not tested
	# path('home_page_owner/<int:o_id>', views.StoreListView.as_view(), name='store-owner-detail'),
	path('add_item_to_store/<int:int>/<slug:id>', views.item_added_ssuccefuly, name='item-detail'),
